...
# no-pool = same estimator fed only target arm (K=1 for estimator)
est_real_nopool = function(Y_list, k, r, x, y, N1_vec, T1_vec) {
  mu4block_pool(
    Y = list(Y_list[[k]]),
    k = 1, r = r, x = x, y = y,
...
run_one_config = function(N, Tt, K, r, SNR_target, nrep,
  ...
  methods = c("real_pool", "real_nopool", "oracle_pool", "oracle_nopool", "oracle_local")
  ...
  for (rep_id in 1:nrep) {
    ...
    mu_r1 = safe_mu(est_real_nopool(dat$Y_list, target_arm, r, x, y, N1_vec, T1_vec))
    mu_op = safe_mu(est_oracle_pool(dat$Y_list, target_arm, dat$U, dat$V, dat$R_list, x, y, N1_vec, T1_vec))
    mu_o1 = safe_mu(est_oracle_nopool(dat$Y_list, target_arm, dat$U, dat$V, dat$R_list, x, y, N1_vec, T1_vec))
    mu_oL = safe_mu(est_oracle_local(dat$Y_list, target_arm, dat$U, dat$V, dat$R_list, x, y, N1_vec, T1_vec))
    
    mu_map = list(real_pool = mu_rp, real_nopool = mu_r1, oracle_pool = mu_op,
                  oracle_nopool = mu_o1, oracle_local = mu_oL)
...
plot_mse_vs_K_pretty = function(sumdf, raw_df, file_png, main_title = NULL) {
  ...
  methods = c("oracle_pool", "oracle_nopool", "oracle_local", "real_pool", "real_nopool")
...
raw_file = file.path("synthetic/result", paste0("raw_compare_pooling_oracle_varyK_randomXY_", tag, ".csv"))
sum_file = file.path("synthetic/result", paste0("summary_compare_pooling_oracle_varyK_randomXY_", tag, ".csv"))
fig_file = file.path("synthetic/figure", paste0("compare_mse_vs_K_pooling_oracle_randomXY_", tag, ".png"))
