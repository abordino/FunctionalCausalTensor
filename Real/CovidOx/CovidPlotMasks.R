# CovidOx: display saved initial and terminal masks ----------------------------
...
# These settings must match the saved simulation.
STEP_SIZE = 1L
JUMP = 30L
REP = 1L
RANK_TO_LOAD = 1L
...
SLICE_LABEL = "until_2020-04-05_delay_28"

RESULT_FILE = NULL

results_dir = file.path(
  "Results",
  "oxford_deaths_target_layer_bootstrap",
...
if (is.null(RESULT_FILE)) {
  RESULT_FILE = file.path(
    results_dir,
    paste0(
      "oxford_deaths_staggered_masking_results_deaths_rank",
      RANK_TO_LOAD,
      "_jump",
      JUMP,
      "_step",
...
if (!file.exists(RESULT_FILE)) {
  stop(
    "Saved CovidOx result file was not found:\n",
    RESULT_FILE
  )
}
...
# M0 is the original target-layer policy-on region.
...
make_mask_plot = function(
  ...
  mask_status[M0] = "Original policy-on"
  ...
  country_levels = rownames(M0)
  
  if (is.null(country_levels)) {
    country_levels = unique(plot_data$country)
  }
  ...
  if (anyNA(plot_data$date)) {
    stop(
      "At least one saved COVID time name could not be converted to Date."
    )
  }
...
for (rep_id in seq_along(mask_sequences)) {
  ...
  initial_plot = make_mask_plot(
    ...
    title = paste0(
      "CovidOx initial artificial mask: replicate ",
      rep_id
    ),
    subtitle = paste0(
      target_label,
...
terminal_plot = make_mask_plot(
  ...
  title = "CovidOx final artificial mask",
  subtitle = paste0(
    target_label,
    "; ",
