...
oracle_muXY4blockLocal = function(Y, k, U, V, R_list,
                             x = NULL, y = NULL,
                             N1 = NULL, T1 = NULL,
                             normalize_xy = TRUE) {
  ...
  # ---- local correction (scalar): A3
  row_start = if (k == 1) 1 else sum(N1[1:(k - 1)]) + 1
  row_end   = sum(N1[1:k])
  U1k = U[1:N1k, 1:r, drop = FALSE]                    # N1k×r
  G_U1_inv = solve(crossprod(U1k))                     # r×r
  E_up_k = E_up_pool[row_start:row_end, , drop = FALSE]  # N1k×T
  v3 = V[, 1:r, drop = FALSE] %*% by                   # T×1
  s3 = crossprod(U1k, E_up_k %*% v3)                   # r×1
  mu_3 = as.numeric(crossprod(x, U2k %*% (G_U1_inv %*% s3)))
